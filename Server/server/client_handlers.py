...
class FTPClientHandler(ClientHandler):
    def __init__(self, logger: logging.Logger) -> None:
        ...
        self.working_wirectory = DEFAULT_DIR
    ...
    def handle_message(self, message: bytes) -> None:
        data = message.decode('utf-8')[:-1]
        command, argument = data[:4].strip().upper(), data[4:].strip() or None
        if command == "QUIT":
            self.QUIT()
        else:
            try:
    ...
    def CWD(self, argument: str) -> None:
        ...
        path = self.working_wirectory/argument
    ...
    def PASV(self) -> None:
        ...
        message = f'227 Entering Passive Mode ({",".join(address.split("."))},{port>>8&0xFF},{port&0xFF}).\r\n'
    ...
    def RETR(self, argument: str) -> None:
        ...
        if self.passive_mode:
            file = self.working_wirectory/argument
            if file.is_file():
                self.logger.debug(
